...
// New create a propertie instance.
func New(cfgDir string, opts ...Option) *Config {
	// must set propertie dir
	if cfgDir == "" {
		panic("propertie dir is not set")
	}
	c := Config{
		configDir:  cfgDir,
		configType: "yaml",
...
func (c *Config) load(filename string, cfgType string) (*viper.Viper, error) {
	...
	if err := v.ReadInConfig(); err != nil {
		if _, ok := err.(viper.ConfigFileNotFoundError); ok {
			return nil, errors.New("propertie file not found")
		}
		return nil, err
	}
...
// Option propertie option
type Option func(*Config)

// WithFileType propertie file type
